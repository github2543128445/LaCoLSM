...
//        main_comm_threads.emplace_back(Clientmessagehandler());
  std::shared_ptr<RDMA_Manager> rdma_mg = env_->rdma_mg;
    ...
    for (int i = 0; i < rdma_mg->compute_nodes.size(); ++i) {//LZYADD,初始化立即数操作的控制变量
      uint8_t target_node_id = 2*i+1;
      if(target_node_id == RDMA_Manager::node_id) continue;
      printf("assign CN_imme node %d\n",target_node_id);
      CN_mtx_imme[target_node_id] = rdma_mg->mtx_imme_map.at(target_node_id);
      CN_imm_gen[target_node_id] = rdma_mg->imm_gen_map.at(target_node_id);
      CN_imme_data[target_node_id] = rdma_mg->imme_data_map.at(target_node_id);
      CN_byte_len[target_node_id] = rdma_mg->byte_len_map.at(target_node_id);
      CN_cv_imme[target_node_id] = rdma_mg->cv_imme_map.at(target_node_id);
    }
...
//    edit->AddFile(level, meta);
  ...
  CompactionStats stats;
  stats.micros = env_->NowMicros() - start_micros; //micros反映SST的大小，衡量了键值对的个数 -LZY
  stats.bytes_written = meta->file_size;
  stats_[level].Add(stats);
//  write_stall_mutex_.AssertNotHeld();
  return s;
...
//    std::unique_lock<std::mutex> l(imm_.imm_mtx);
  ...
  DEBUG_arg("picked metable number is %lu\n", f_job.mem_vec.size());
...
// by the thread which CAS the memtable successfully.
 if (shutting_down_.load(std::memory_order_acquire)) {
  ...
  if (imm_.IsFlushPending()) {//如果准备了待flush的immutable table -LZY
//    background_compaction_scheduled_ = true;
    ...
    env_->Schedule(BGWork_Flush, static_cast<void*>(thread_pool_args), type);//返回相应线程池的线程个数
...
void DBImpl::BGWork_Flush(void* thread_arg) {//触发flush-LZY
...
Status DBImpl::DoRemoteCompactionWork3(CompactionState* compact,uint8_t target_node_id,uint64_t start_num){//完全复用CN
  ...
  // }
  //printf("DoRemoteCompactionWork:cp0\n");
  int cases = compact->compaction->WhatCase(options_.usesubcompaction);
...
Status DBImpl::DoRemoteCompactionWorkWithSubcompaction(CompactionState* compact,uint8_t target_node_id,uint64_t start_num){//LZYADD
  ...
  if (boundaries->size()<=options_.max_remote_subcompactions){
    ...
    int files_per_subcompaction = big_files_num/options_.max_remote_subcompactions + 1;//Due to interger round down, we need add 1.
    double mean = sum * 1.0 / options_.max_remote_subcompactions;
    for (size_t i = 0; i <= boundaries->size(); i++) {
...
//      }
  ...
  for (size_t i = 1; i < compact->sub_compact_states.size(); i++) {
    thread_pool.emplace_back(&DBImpl::RemoteProcessKeyValueCompactionPlusCases, this,
                             &compact->sub_compact_states[i],target_node_id,remote_file_num,cases);
  }

  // Always schedule the first subcompaction (whether or not there are also
  // others) in the current thread to be efficient with resources
  RemoteProcessKeyValueCompactionPlusCases(&compact->sub_compact_states[0],target_node_id,remote_file_num,cases);
...
//  assert(compact->outfile == nullptr);
  // if (snapshots_.empty()) { //这是写操作的序列号, 这应该不能用本地的判断吧
  //   compact->smallest_snapshot = versions_->LastSequence();
  // } else {
  //   compact->smallest_snapshot = snapshots_.oldest()->sequence_number();
  // }LZYDEL感觉不用管，因为MN没管
...
void DBImpl::Other_Compaction_Handler1(void* arg){//参考Memory_Node_Keeper::sst_compaction_handler
  printf("Other_Compaction_Handler:cp0\n");
  RDMA_Request* request = ((Arg_for_handler*) arg)->request;
  std::string client_ip = ((Arg_for_handler*) arg)->client_ip;
  uint8_t target_node_id = ((Arg_for_handler*) arg)->target_node_id;
...
void DBImpl::QuickSubLocalCompactionThread(){
  // if(options_.now_local_compactions <= options_.min_local_compactions) return;
  // options_.now_local_compactions -=2;
  // if(options_.now_local_compactions < options_.min_local_compactions) options_.now_local_compactions = options_.;
...
bool DBImpl::CheckWhetherPushDownorNot(Compaction* compact) { //not use
...
int DBImpl::CompactionTaskWhereToGo(Compaction* compact){//最原始版本，弃用
#if NEARDATACOMPACTION==2
  auto rdma_mg = env_->rdma_mg;
  int aim = 0;
  ...
  if (compact->level() == 0){//Level 0
    if(options_.usesubcompaction && compact->CanSubCompaction()){ //L0 + Sub
      ...
      RMN_Score = 1.5 * RMN_now_achievable_parallel;//不分大小任务，因为MN连多个CN，不可能有机会all in           
      //RMN + L0 + Sub ↑
      ...
      for(auto it : RCN_core){
        ...
        double RCN_temp_Score = 0.7*RCN_v_av_core;//LZYTODO,应该差一些
        if(RCN_temp_Score > RCN_Score){
          RCN_best_id = it.first;
          RCN_Score = RCN_temp_Score;
        }
    ...
    if(options_.usesubcompaction && compact->CanSubCompaction()){//LN + Sub
      ...
      RMN_Score = 1.5 * RMN_now_achievable_parallel;      
      //RMN + LN + Sub ↑
...
int DBImpl::CompactionTaskWhereToGoTestv0(Compaction* compact){//LZYTODO
#if NEARDATACOMPACTION==2
  auto rdma_mg = env_->rdma_mg;
  int aim = 0;
  int L0_num = compact->num_input_files(0),L1_num = compact->num_input_files(1);
  ...
  //                         options_.max_memory_compactions + //总Compaction线程
  //                         options_.max_memory_subcompactions; //赋权的SubCompaction线程
  double Local_v_core = Local_core;//开多少是多少
  ...
  if (compact->level() == 0){//Level 0
    if(options_.usesubcompaction && compact->CanSubCompaction()){ //L0 + Sub
      ...
      for(auto it : RCN_core){
        ...
        double RCN_max_achievable_parallel = options_.max_compute_subcompactions < task_parallelism ? options_.max_compute_subcompactions : task_parallelism;
        double RCN_now_achievable_parallel = RCN_max_achievable_parallel < RCN_v_av_core ? RCN_max_achievable_parallel : RCN_v_av_core;
        double RCN_temp_Score = 0.63*RCN_now_achievable_parallel;//LZYTODO,应该差一些
        if(RCN_temp_Score > RCN_Score){
          RCN_best_id = it.first;
          RCN_Score = RCN_temp_Score;
        }
      ...
      for(auto it : RCN_core){
        ...
        double RCN_temp_Score = 0.85*RCN_v_av_core;
        if(RCN_temp_Score > RCN_Score){
          RCN_best_id = it.first;
          RCN_Score = RCN_temp_Score;
        }
...
int DBImpl::CompactionTaskWhereToGoTestv1(Compaction* compact){//LZYTODO
#if NEARDATACOMPACTION==2
  auto rdma_mg = env_->rdma_mg;
  int aim = 0;
  int L0_num = compact->num_input_files(0),L1_num = compact->num_input_files(1);
...
int DBImpl::CompactionTaskWhereToGoTestv2(Compaction* compact){//该版本直接将case3，case4给自己
#if NEARDATACOMPACTION==2
  auto rdma_mg = env_->rdma_mg;
  int aim = 0;
  int L0_num = compact->num_input_files(0),L1_num = compact->num_input_files(1);
...
int DBImpl::CompactionTaskWhereToGoTestv4(Compaction* compact){//结合pickv3，case1 2在C0C1中抉择，case3 4在C0C2中抉择
#if NEARDATACOMPACTION==2
  auto rdma_mg = env_->rdma_mg;
  int aim = 0;
  int L0_num = compact->num_input_files(0),L1_num = compact->num_input_files(1);
  printf("CompactionTaskWhereToGo : L0_num = %d, L1_num = %d\n", L0_num, L1_num);
  uint16_t Local_core = rdma_mg->local_compute_core_number;
  uint16_t RMN_core = rdma_mg->remote_core_number_map.at(shard_target_node_id);
  ...
  printf("CompactionTaskWhereToGo : Local core = %d, utilization = %f, RMN core = %d, utilization = %f\n", Local_core, Local_utilization, RMN_core, RMN_utilization);
    ...
    if(options_.usesubcompaction && compact->CanSubCompaction()){//LN + Sub
      ...
      //RCN + LN + Sub ↑
      printf("Compaction Task Score 3: Local: %f, RMN: %f, RCN: %f\n", Local_Score, RMN_Score, RCN_Score);
      // if(RMN_Score > Local_Score && RMN_Score > RCN_Score && L1_num <= 4){
      //   aim = shard_target_node_id;
      // }else 
      if(Local_Score > RCN_Score){
        aim = -1;
      }else {
        aim = RCN_best_id;
      }     
...
Status DBImpl::OpenCompactionOutputFileFor(CompactionState* compact,uint8_t target_node_id){
  ...
  uint64_t file_number;
  {
    ...
    // file_number = versions_->NewFileNumber();//LZYCONTINUE CN新建时记录, MN不记录, 随便改改试试吧
  ...
  if (s.ok()) {
    if (compact->compaction->table_type == block_based){//never block_based
      compact->builder = new TableBuilder_ComputeSide(options_, Compact, target_node_id);
    }else{
  ...
  if (compact->sub_compact_states.size() == 0){//
    for (size_t i = 0; i < compact->outputs.size(); i++) {//LZY:不含SubCompaction
      ...
      std::shared_ptr<RemoteMemTableMetaData> meta = std::make_shared<RemoteMemTableMetaData>(0,table_cache_,
                                                   shard_target_node_id);//LZYTODO需要修改id
      //TODO make all the metadata written into out
      meta->number = out.number;//MN无
      meta->level = level+1;
      meta->file_size = out.file_size;
...
void DBImpl::RemoteDataCompaction(Compaction* c,uint8_t target_node_id){//参考DBImpl::NearDataCompaction
  std::shared_ptr<RDMA_Manager> rdma_mg = env_->rdma_mg;
  int cases = c->WhatCase(options_.usesubcompaction);
  auto start_time = std::chrono::steady_clock::now();
  // register the memory block from the remote memory
  ...
  send_pointer->command = remote_data_compaction; //LZYCHA
  send_pointer->content.sstCompact.buffer_size = serilized_c.size() + 1;
  send_pointer->buffer = receive_mr.addr;
  send_pointer->rkey = receive_mr.rkey;
  send_pointer->buffer_large = mr_c.addr;//到时候发回的时候就在mr_c了
  ...
  printf("send compaction at level %d, %d files in 1st level, %d files in 2ed level\n", c->level(), c->num_input_files(0), c->num_input_files(1));
  ...
  auto end_time = std::chrono::steady_clock::now();
  C2_append(std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count(),cases,0);//发送耗时
  start_time = std::chrono::steady_clock::now();
  ...
  size_t counter = 0;
  std::unique_lock<std::mutex> lck(*CN_mtx_imme[target_node_id]);
  //完成任务后对方发回imm_num以及元数据修改
  while (imm_num != *CN_imme_data[target_node_id]){//等待任务编号的回收
    CN_cv_imme[target_node_id]->wait(lck);
  }
  end_time = std::chrono::steady_clock::now();
  C2_append(std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count(),cases,1);//等待处理耗时
  start_time = std::chrono::steady_clock::now();
  ...
  *CN_imme_data[target_node_id] = 0;
  ...
  VersionEdit edit(0);
  edit.DecodeFrom(Slice((char*)mr_c.addr, buffer_size), 0, table_cache_);//从mr_c获得元数据的修改
  ...
  edit.GetNewFiles();
  end_time = std::chrono::steady_clock::now();
  C2_append(std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count(),cases,2);//处理写回的元数据
  {
...
//    lck_vs.unlock();
    ...
    InstallSuperVersion();
    printf("RemoteDataCompaction cp6\n");
    write_stall_cv.notify_all();
  }
  ...
  printf("RemoteDataCompaction MY %d end\n",imm_num);
}
...
//             sizeof(receive_mr.addr) + sizeof(receive_mr.rkey), 1, 1);
  ...
  *receive_pointer = {};
  ...
  auto end_time = std::chrono::steady_clock::now();
  C1_append(std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count(),cases,0);
  start_time = std::chrono::steady_clock::now();
#ifdef WITHPERSISTENCE
...
//  *polling_size_2 = 0;
  ...
  end_time = std::chrono::steady_clock::now();
  C1_append(std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count(),cases,1);
  start_time = std::chrono::steady_clock::now();
...
//  }
  ...
  edit.GetNewFiles();
  end_time = std::chrono::steady_clock::now();
  C1_append(std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count(),cases,2);
  {
    std::unique_lock<std::mutex> sv_lck(superversion_memlist_mtx);
    // TODO: remove the version id argument because we no longer need it.
//    std::unique_lock<std::mutex> lck_vs(versionset_mtx, std::defer_lock);
...
//      assert(locked == false);
      ...
      if (imm_.current_memtable_num() <= config::Immutable_StopWritesTrigger&&
        ...
        temp_mem->SetLargestSeq(last_mem_seq + MEMTABLE_SEQ_SIZE);
        temp_mem->Ref();
        mem_r->SetFlushState(MemTable::FLUSH_REQUESTED);//写新memtable。为旧memtable准备flush-LZY
        mem_.store(temp_mem);
        //set the flush flag for imm
        assert(imm_.current_memtable_num() <= config::Immutable_StopWritesTrigger);
        imm_.Add(mem_r);
