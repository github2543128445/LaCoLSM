...
//  static bool check_compaction_state(std::shared_ptr<RemoteMemTableMetaData> sst);
  bool PickLevel0FilePlanv0(int level, Compaction* c, Version* current_snap); //LZYADD Default
  void PickLevelNFilePlanv0(int level, Compaction* c, Version* current_snap); //LZYADD Default
  ...
  //bool PickLevel0FilePlanv0B(int level, Compaction* c, Version* current_snap); LZYADD废案，无论如何L0总是重叠的，精挑细选没有意义
