...
static double MaxBytesForLevel(const Options* options, int level) { //level1是256MB，后每层×10
...
bool Version::GetOverlappingInputs(int level, const InternalKey* begin,//LZY：level i层的工作有UnderCompaction的时候返回false
...
void VersionSet::Finalize(Version* v) {  //为下次Compaction计算最好的level 放在v->compaction_level_[0]  -LZY  
...
//  double best_score = -1;
  ...
  for (int level = 0; level < config::kNumLevels - 1; level++) {
    ...
    if (level == 0) {
      ...
      score = (v->levels_[level].size() - v->in_progress[level].size())/   //（文件数量-需要Compaction的文件数量）/ 1 -LZY 
...
//    }
  ...
  int level = v->compaction_level_[0];
  //printf("///level %d got %lf, has %d files and %d files need compacrtion\n\ttotal filesize=%lldB and %lldB needs compaction\n\t",
  //      level,v->compaction_score_[0], v->levels_[level].size(),v->in_progress[level].size(),TotalFileSize(v->levels_[level]),TotalFileSize(v->in_progress[level]));
  //排完分数大的在前。
    ...
    if (!c->inputs_[which].empty()) {
      if (c->level() + which == 0) {
        ...
        list[num++] = NewTwoLevelFileIterator(
            ...
            &GetFileIterator, table_cache_, options);//也会走Cache开销忽略不计
      }
    }
  ...
  for (int which = 0; which < 2; which++) {
    if (!c->inputs_[which].empty()) {
      if (c->level() + which == 0) {
        ...
        list[num++] = NewTwoLevelFileIterator(
            new Version::LevelFileNumIterator(icmp_, &c->inputs_[which]),
            &GetFileIterator_Memoryside, table_cache_, options);//开销忽略不计
      }
    }
  }
  assert(num <= space);
...
// bool VersionSet::PickLevel0FilePlanv0B(int level, Compaction* c, Version* current_snap){ //LZYADD 废案，L0总是重叠，精挑细选没有意义
...
//   if(l1size == 0) return PickLevel0FilePlanv0(level, c,current_snap);
...
//           if(c->inputs_[0].size()==0) return PickLevel0FilePlanv0(level, c,current_snap);//无重叠，用老方法
...
// }
bool VersionSet::PickLevel0FilePlanv0(int level, Compaction* c, Version* current_snap){//LZY:default
...
void VersionSet::PickLevelNFilePlanv0(int level, Compaction* c, Version* current_snap){//default
...
void VersionSet::PickLevelNFilePlanv2(int level, Compaction* c, Version* current_snap){
  size_t current_level_size = current_snap->levels_[level].size();
  size_t random_index = std::rand() % current_level_size;
  InternalKey smallest, largest;
  auto user_cmp = icmp_.user_comparator();
  ...
  while (1) {
    ...
    if (!f->UnderCompaction) {
      ...
      if(level == 1){
        int cnt = 1;
        while (random_index + cnt < current_level_size){//可调节参数，代表input i的数量
          std::shared_ptr<RemoteMemTableMetaData> next_f = current_snap->levels_[level][random_index + cnt];
          if(!next_f->UnderCompaction && (user_cmp->Compare(next_f->smallest.user_key(), f->largest.user_key()) == 0 || cnt < 4) ){ 
            c->inputs_[0].push_back(next_f);       
            cnt++;                     
          }else{
            printf("PickLevelNFilePlanv2: L1 next UnderCompaction\n");
            break;
          }
        }
      }else{//L1+N
        int cnt = 1;
        while (random_index + cnt < current_level_size){
          std::shared_ptr<RemoteMemTableMetaData> next_f = current_snap->levels_[level][random_index + cnt];
          if(!next_f->UnderCompaction && user_cmp->Compare(next_f->smallest.user_key(), f->largest.user_key()) == 0){
            c->inputs_[0].push_back(next_f);  
            printf("FUCKYOU\n");   
            cnt++;                      
          }else{
            printf("PickLevelNFilePlanv2: LN next UnderCompaction\n");
            break;
          }
        }
      }       
...
void VersionSet::PickLevelNFilePlanv3(int level, Compaction* c, Version* current_snap){
  size_t current_level_size = current_snap->levels_[level].size();
  size_t random_index = std::rand() % current_level_size;
  InternalKey smallest, largest;
  auto user_cmp = icmp_.user_comparator();
  ...
  while (1) {
    ...
    if (!f->UnderCompaction) {
      ...
      if(level == 1){
        ...
        while (random_index + cnt < current_level_size){//可调节参数，代表input i的数量
          std::shared_ptr<RemoteMemTableMetaData> next_f = current_snap->levels_[level][random_index + cnt];
          if(!next_f->UnderCompaction && (user_cmp->Compare(next_f->smallest.user_key(), f->largest.user_key()) == 0 || cnt <= barry) ){ 
            c->inputs_[0].push_back(next_f);       
            cnt++;                     
          }else{
            break;
...
#if NEARDATACOMPACTION == 2 //LZYTODO
  if(level == 0){
...
Compaction* VersionSet::PickCompaction(std::mutex* sv_mtx_within_function) {   //找到需要Compaction的层和SST,存到c里return
  ...
  for (int i = 0; i < config::kNumLevels - 1; i++) {
    ...
    if (level_score >= 1){
      ...
      if (skipped_l0_to_base && level == 1) { // LZY : Level 0先来，且有level 1的Compaction，忽略level 1的
      ...
      if (PickFileToCompact(level, c, current_snap)) {//LZY：得到需要Compaction的文件放在current_snap->in_progress[]里
        assert(c->level() == level && level < 10);
#ifndef NDEBUG
        for (auto iter : c->inputs_[0]) {
          assert(std::find(current_snap->levels_[level].begin(), current_snap->levels_[level].end(), iter) //LZY：记得关
...
//    }
    return c;//LZY：返回DBImpl::BackgroundCompaction
  }else{
    delete c;
    return nullptr;
  }
