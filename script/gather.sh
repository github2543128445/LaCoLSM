...
echo "处理data-node2目录..."
cp "../${filename}*.csv" ../data/data-node2/ 2>/dev/null || echo "提示: 未找到${filename}*.csv文件"

# 从skv-node3复制CSV文件
echo "从skv-node3获取CSV..."
scp "skv-node3:~/louzy/LaCoLSM/${filename}*.csv" ../data/data-node3/ 2>/dev/null || echo "提示: skv-node3无${filename}*.csv文件"
